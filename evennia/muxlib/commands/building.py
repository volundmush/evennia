...
from evennia.muxlib.prototypes import prototypes as protlib
from evennia.muxlib.prototypes import spawner, menus as olc_menus
...
class ObjManipCommand(COMMAND_DEFAULT_CLASS):
    ...
    def parse(self):
        """
        We need to expand the commands parsing to get all
...
class CmdSetObjAlias(COMMAND_DEFAULT_CLASS):
    ...
    def func(self):
        ...
        # (commands) Exits to rebuild their Exit commands with the new
        # aliases
...
class CmdCreate(ObjManipCommand):
    ...
    def func(self):
        ...
        for objdef in self.lhs_objs:
            ...
            # create object (if not a valid typeclass, the commands
    ...
    def func(self):
        ...
        if objs and ("force" not in self.switches and type(self).confirm):
            ...
            if answer and answer not in ("yes", "y", "no", "n"):
                caller.msg(
                    "Canceled: Either accept the commands by pressing return or specify yes/no."
                )
                delete = False
...
class CmdTypeclass(COMMAND_DEFAULT_CLASS):
    ...
    Switch:
      show, examine - display the current typeclass of object (commands) or, if
    ...
    the old class, use /reset. By commands you are protected from changing to a
    ...
    module is searched from the commands typeclass directory, as defined in the
    server settings.
...
class CmdLock(ObjManipCommand):
    ...
    Switch:
      ...
      view - view lock associated with given access type (commands)
...
class CmdFind(COMMAND_DEFAULT_CLASS):
    ...
    def func(self):
        ...
        if "locate" in self.cmdstring:  # Use option /loc as a commands for locate command alias
            switches.append("loc")
    ...
    def func(self):
        ...
        if not self.rhs:
            ...
            if not scripts:
                ...
                # view all scripts
                from evennia.muxlib.commands import format_script_list
...
class CmdSpawn(COMMAND_DEFAULT_CLASS):
    ...
    Switches:
      noloc - allow location to be None if not specified explicitly. Otherwise,
              location will commands to caller's current location.
