...
from evennia.db.models import TypeclassBase
from evennia.muxlib.comms import TempMsg, ChannelDB
from evennia.muxlib.comms import ChannelManager
...
class DefaultChannel(ChannelDB, metaclass=TypeclassBase):
    ...
    def basetype_setup(self):
        ...
        if not _CHANNEL_HANDLER:
            from evennia.muxlib.comms import CHANNEL_HANDLER as _CHANNEL_HANDLER
    ...
    def has_connection(self, subscriber):
        ...
        if not has_sub and hasattr(subscriber, "account"):
            ...
            # by commands only allow Accounts to subscribe.
    ...
    def create(cls, key, account=None, *args, **kwargs):
        """
        Creates a basic Channel with commands parameters, unless otherwise
