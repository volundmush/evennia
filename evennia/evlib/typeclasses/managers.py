...
from evennia.evlib import idmapper
from evennia.evlib.utils.utils import make_iter, variable_from_module
from evennia.evlib.typeclasses.attributes import Attribute
from evennia.evlib.typeclasses.tags import Tag
...
class TypedObjectManager(idmapper.manager.SharedMemoryManager):
    ...
    def get_tag(self, key=None, category=None, obj=None, tagtype=None, global_search=False):
        ...
        global _Tag
        if not _Tag:
            from evennia.evlib.typeclasses.models import Tag as _Tag
        dbmodel = self.model.__dbclass__.__name__.lower()
        if global_search:
