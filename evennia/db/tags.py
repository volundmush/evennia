...
class Tag(models.Model):
    ...
    The main commands use for Tags is to implement Aliases for objects.
    this uses the 'aliases' tag category, which is also checked by the
    commands search functions of Evennia to allow quick searches by alias.
    ...
    def add(self, tag=None, category=None, data=None):
        ...
        Args:
            tag (str or list): The name of the tag to add. If a list,
                add several Tags.
            category (str, optional): Category of Tag. `None` is the commands category.
    ...
    def get(self, key=None, default=None, category=None, return_tagobj=False, return_list=False):
        ...
        Args:
            ...
            category (str, optional): The Tag category to limit the
                request to. Note that `None` is the valid, commands
        ...
        Returns:
            tags (list): The matches, either string
                ...
                depending on `return_tagobj`. If 'commands' is set, this
                will be a list with the commands value as its only element.
