...
class TestLauncher(TwistedTestCase):
    def test_is_windows(self):
        self.assertEqual(launcher._is_windows(), os.name == "nt")

    def test_file_compact(self):
        self.assertEqual(
            launcher._file_names_compact("foo/bar/test1", "foo/bar/test2"),
            "foo/bar/test1 and test2",
        )

        self.assertEqual(
            launcher._file_names_compact("foo/test1", "foo/bar/test2"),
    ...
    def test_print_info(self, mockprint):
        ...
        launcher._print_info(portal_dict, server_dict)
        mockprint.assert_called()

    def test_parse_status(self):
        response = {"status": pickle.dumps(("teststring",))}
        result = launcher._parse_status(response)
    ...
    def test_get_twisted_cmdline(self):
        pcmd, scmd = launcher._get_twistd_cmdline(False, False)
        ...
        pcmd, scmd = launcher._get_twistd_cmdline(True, True)
        self.assertTrue("portal.py" in pcmd[1])
        self.assertTrue("--pidfile" in pcmd[2])
    ...
    def test_query_status_callback(self):
        ...
        launcher.query_status(callback=testcall)
    ...
    def test_wait_for_status_reply(self, mprint, aconn):
        ...
        launcher.wait_for_status_reply(test)
    ...
    def test_wait_for_status_reply_fail(self, mprint):
        launcher.wait_for_status_reply(None)
