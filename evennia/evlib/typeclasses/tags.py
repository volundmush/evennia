...
from evennia.evlib.utils.utils import to_str, make_iter
...
class Tag(models.Model):
    ...
    db_key = models.CharField(
        "key", max_length=255, null=False, help_text="tag identifier", db_index=True
    )
    db_category = models.CharField(
        "category", max_length=64, null=False, help_text="tag category", db_index=True
    )
    ...
    db_model = models.ForeignKey(ContentType, on_delete=models.CASCADE)
    db_object_id = models.PositiveIntegerField()
    db_owner = GenericForeignKey('db_model', 'db_object_id')

    # this is tag, alias or permission
    ...
    class Meta(object):
        ...
        verbose_name = "Tag"
        unique_together = (("db_key", "db_category", "db_tagtype", "db_model", "db_object_id"),)
...
class TagHandler(object):
    ...
    def __init__(self, obj):
        ...
        """
        self.obj = obj
        self._storage = self.obj.tag_storage
        self._cache = {}
    ...
    def _query_all(self):
        ...
        return [
            conn.tag
            for conn in self._storage.filter(**query)
        ]
    ...
    def _getcache(self, key=None, category=None):
        ...
        if key:
            ...
            if tag:
                ...
                conn = self._storage.filter(**query)
                if conn:
                    tag = conn[0].tag
            ...
            if _TYPECLASS_AGGRESSIVE_CACHE and catkey in self._catcache:
                ...
                tags = [conn.tag for conn in self._storage.filter(**query)]
                if _TYPECLASS_AGGRESSIVE_CACHE:
                    for tag in tags:
    ...
    def add(self, tag=None, category=None, data=None):
        ...
        for tagstr in make_iter(tag):
            ...
            tagobj = self._storage.create(key=tagstr, category=category, data=data, tagtype=self._tagtype)
    ...
    def remove(self, key=None, category=None):
        ...
        for key in make_iter(key):
            ...
            tagobj = self._storage.filter(db_key=tagstr, db_category=category, db_tagtype=self._tagtype)
            if tagobj:
                tagobj.delete()
    ...
    def clear(self, category=None):
        ...
        self._storage.filter(**query).delete()
        self._cache = {}
        self._catcache = {}
        self._cache_complete = False
